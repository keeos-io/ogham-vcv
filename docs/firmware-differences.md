...
| Input edges at the boundary | An edge is an interrupt; the core is always running | Latched in `RateConverter` and delivered on the next core step. `advance()` returns zero for half the host samples at 96 kHz and three in four at 192 kHz, so an unlatched edge landing on one is dropped where it stands | live | Yes — a bug, fixed 2026-08-29 |
