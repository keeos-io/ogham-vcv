...
| Encoder mode | A 600 ms hold is the only way between function select and the menu, and which mode you are in is only visible on the display | A panel switch, bound both ways: moving it changes mode, and a hold (or the right-click item) moves it. The hold still works — it is no longer the only way, and no longer invisible | live | Yes. A hold is a fine gesture with a thumb and a poor one with a mouse, where it is also the action that begins a turn |
| Encoder gestures | Turning and pressing are physically separate: fingers on the shaft, thumb on the button | One mouse button does both, so the gesture is decided by what the pointer does — movement makes it a turn and no press is reported at all, holding still past 140 ms makes it a press, releasing before either emits a click | live | Yes |
